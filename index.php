...
if ($username) {
	...
	$unread = 0; //signifies how many times we've been through the loop -- 1 to check unread posts, 2 for the others.
	while ($unread != 2) {
		$tdspacer = "****";
		if ($unread == 0) {	
			$unread = 1;
			...
			if ($num_rows == 0) {
				...
				echo "<form name=\"markasread\" action=\"index.php\" method=\"POST\"><input type=\"hidden\" name=\"markasread\"><input type=\"hidden\" name=\"readdate\" value=\"" . time() . "\">
		...
		while($convs = mysql_fetch_array($res)) {
			...
			if ($unread == 1) {
				echo "<tr$rowcolor>
						<td><img src=\"gfx/new.gif\" border=0 width=31 height=12 hspace=8></td>
						<td class=\"rowpad\"><a href=\"conversations.php?id=$convid&new=1\">$contitle</a> ($numcomm)</td>
